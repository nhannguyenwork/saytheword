...
import { MousePointer2, Trash2, History, CheckCircle2, XCircle, Trophy, RefreshCcw, Eye, EyeOff, Play, Layers, Gamepad2, ArrowRight, ArrowLeft, Star, Timer } from 'lucide-react';
...
export default function App() {
  ...
  // Thực sự bắt đầu chơi game (bấm nút Bắt đầu)
  const startGame = () => {
  ...
  const editSlot = (index: number) => {
    if (!selectedLevel || gameStatus !== 'PLAYING') return;
    const currentHistory = roundsHistory[currentRoundIndex];
    
    if (index >= currentHistory.length) return;

    const newRoundsHistory = [...roundsHistory];
    const newHistory = [...currentHistory];
    newHistory[index] = newHistory[index] === "Mèo" ? "Chó" : "Mèo";
    newRoundsHistory[currentRoundIndex] = newHistory;
    setRoundsHistory(newRoundsHistory);
  };
  ...
  const checkResult = () => {
    if (!selectedLevel || !startTime) return;
  ...
  const isAllRoundsComplete = selectedLevel && roundsHistory.every(round => round.length === 8);
  ...
  return (
    <div className="fixed inset-0 bg-slate-50 text-slate-900 font-sans flex flex-col items-center p-2 md:p-8 overflow-y-auto overflow-x-hidden">
      ...
      {/* Overlay Bắt đầu game */}
      {gameStatus === 'IDLE' && selectedLevel && (
        <div className="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4">
          <motion.div 
            initial={{ scale: 0.8, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="bg-white rounded-[2rem] md:rounded-[3rem] p-8 md:p-12 shadow-2xl text-center max-w-md w-full border-4 border-red-100"
          >
            <div className="w-20 h-20 bg-red-600 rounded-3xl flex items-center justify-center text-white mx-auto mb-6 shadow-xl shadow-red-200">
              <Play size={40} fill="currentColor" />
            </div>
            <h2 className="text-3xl md:text-4xl font-black text-slate-800 uppercase italic mb-2">{selectedLevel.name}</h2>
            <p className="text-slate-500 font-bold mb-8 uppercase tracking-widest text-sm">Sẵn sàng để thử thách trí nhớ?</p>
            <button 
              onClick={startGame}
              className="w-full py-4 bg-red-600 text-white font-black text-xl md:text-2xl rounded-2xl shadow-[0_8px_0_0_rgba(185,28,28,1)] hover:translate-y-[-2px] active:translate-y-[4px] active:shadow-none transition-all cursor-pointer uppercase tracking-tighter"
            >
              Bắt đầu ngay
            </button>
            <button 
              onClick={resetToMenu}
              className="mt-4 text-slate-400 font-bold hover:text-slate-600 transition-colors uppercase text-xs tracking-widest"
            >
              Quay lại chọn đề
            </button>
          </motion.div>
      ...
      <div className="w-full max-w-4xl mt-2 md:mt-4 mb-4 md:mb-8 text-center">
        ...
        <div className="bg-white border-2 md:border-4 border-red-100 rounded-2xl md:rounded-[2rem] p-3 md:p-6 shadow-xl relative overflow-hidden mx-2">
          ...
          <AnimatePresence mode="popLayout" custom={direction}>
            ...
            >
              {/* Hiển thị các ô ký tự sau khi nhập - Grid 4x2 */}
              <div className="grid grid-cols-4 gap-2 md:gap-4 mt-4 md:mt-6 max-w-fit mx-auto">
                {currentRoundSequence.map((_, idx) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`w-12 h-14 md:w-16 md:h-20 rounded-lg md:rounded-xl border-2 flex items-center justify-center font-black text-sm md:text-xl transition-all duration-200
                        ${word 
                          ? (word === "Mèo" ? 'bg-red-600 border-red-700 text-white shadow-md cursor-pointer hover:scale-105 active:scale-95' : 'bg-slate-900 border-slate-950 text-white shadow-md cursor-pointer hover:scale-105 active:scale-95')
      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl md:rounded-[2.5rem] p-4 md:p-8 shadow-2xl border-2 md:border-4 border-slate-100 mb-4 md:mb-8 relative mx-2">
        ...
        {gameStatus !== 'RESULT' && (
          <div className="mt-4 md:mt-8 flex flex-col items-center gap-2">
            {!isAllRoundsComplete && (
              <p className="text-slate-400 font-bold text-[10px] md:text-xs uppercase tracking-widest animate-pulse">
                Hoàn thành tất cả 5 round để kiểm tra
              </p>
            )}
            <button 
              onClick={checkResult}
              disabled={!isAllRoundsComplete}
              className={`px-8 py-3 md:px-12 md:py-4 font-black text-lg md:text-2xl rounded-xl md:rounded-2xl transition-all flex items-center gap-2 md:gap-3 cursor-pointer
                ${isAllRoundsComplete 
                  ? 'bg-emerald-500 text-white shadow-[0_4px_0_0_rgba(16,185,129,1)] md:shadow-[0_8px_0_0_rgba(16,185,129,1)] hover:translate-y-[-2px] active:translate-y-[4px] active:shadow-none' 
                  : 'bg-slate-200 text-slate-400 cursor-not-allowed opacity-50'
                }
              `}
            >
              <CheckCircle2 size={20} /> KẾT THÚC GAME
            </button>
          </div>
        )}
        ...
        {gameStatus === 'RESULT' && (
          ...
          `}>
            <div className="flex flex-col items-center mb-4 md:mb-8">
              ...
              {totalTime && (
                <div className="mt-2 flex items-center gap-2 text-slate-500 font-black uppercase tracking-widest text-xs md:text-lg">
                  <Timer size={18} /> Thời gian: {formatTime(totalTime)}
                </div>
              )}
            </div>

            {/* Chi tiết đối chiếu đáp án */}
            <div className="w-full max-w-2xl mb-8 space-y-4">
              <h3 className="text-center font-black text-slate-400 uppercase tracking-widest text-xs md:text-sm mb-4">Đối chiếu chi tiết</h3>
              {selectedLevel.rounds.map((round, rIdx) => (
                <div key={rIdx} className="bg-white p-3 md:p-4 rounded-xl border border-slate-200">
                  <div className="text-[10px] md:text-xs font-black text-slate-400 mb-2 uppercase">Round {rIdx + 1}</div>
                  <div className="grid grid-cols-8 gap-1 md:gap-2">
                    {round.map((target, cIdx) => {
                      const player = roundsHistory[rIdx][cIdx];
                      const isCorrect = player === target;
                      return (
                        <div key={cIdx} className="w-full aspect-[3/4] relative">
                          {isCorrect ? (
                            <div className="w-full h-full rounded md:rounded-lg bg-emerald-500 border border-emerald-600 flex items-center justify-center text-[8px] md:text-xs font-black text-white shadow-sm">
                              {player}
                            </div>
                          ) : (
                            <div className="w-full h-full rounded md:rounded-lg border border-slate-200 overflow-hidden flex flex-col shadow-sm">
                              <div className="flex-1 bg-red-500 flex items-center justify-center text-[8px] md:text-xs font-black text-white border-b border-white/20">
                                {player}
                              </div>
                              <div className="flex-1 bg-emerald-500 flex items-center justify-center text-[8px] md:text-xs font-black text-white">
                                {target}
                              </div>
                            </div>
                          )}
